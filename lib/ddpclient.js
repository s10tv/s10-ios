...
class TSDDPClient {
  constructor(wsurl) {
    ...
    // this.ddpClient.on('message', function (msg) {console.log("ddp message: " + msg); });
  ...
  call(options) {
    ...
    if (!methodName) {
      this.logger.error('Cannot call method without method name');
    }

    if (params && !_.isArray(params)) {
      this.logger.warning('Params must be passed as an array to ddp.call');
    }
