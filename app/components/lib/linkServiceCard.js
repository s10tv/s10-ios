...
function linkSingleIntegrationCard(integration, onPressToLink) {
  ...
  return (
    <TappableCard key={id} onPress={() => onPressToLink(integration.url)}>
      { renderCardInfo(icon.url, status, name, serviceUsername) }
    </TappableCard>
  )
}

export default function linkServiceCard(integrations, onLinkFacebook, onLinkViaWebView) {
  ...
  return (
    <View style={styles.cards}>
      { integrations.map(integration => {
        const onPressToLink = integration.name == 'facebook' ? onLinkFacebook : onLinkViaWebView;
